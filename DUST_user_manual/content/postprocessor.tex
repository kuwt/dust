...
\todo{insert the number of the following lines as field for parsing}.
As an example, the output file for a 10-section blade reads
\begin{inputfile}[frame=single]
 4: # Element Center (3), Element Normal (3), Element Center Velocity (3), 
      Element Area (1), Force Acting on Element (3)
\end{inputfile}
As an example, the lines collecting the fields described above of two components where the first one has 100 elements, reads:
\begin{inputfile}[frame=single]
  5: Comp 000
    ...
  106: Comp 002
