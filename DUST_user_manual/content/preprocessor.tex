...
\item \param{airfoil\_table\_correction}: \textit{required:} no. 
...
Type of subdivision in the chord-wise direction. Can be \opt{uniform} for a uniform distribution, 
\opt{cosine} for a cosine distribution, refined both on the leading and trailing edge, 
\opt{cosineLE} for a half cosine refined only on leading edge or \opt{cosineTE} for a half cosine refined only on the trailing edge. 
...
multiplying by the \param{chord} parameter the coordinates 
...
\textit{default}: \param{referenceLine} , \textit{type}: string.
